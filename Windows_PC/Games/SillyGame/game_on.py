...
def run_game():
	...
	machine = Player("machine", Point(board.convert_xy(8, 7)), RED)
	x1, y1 = 0, board.height / 2
	x3, y3 = board.width, board.height / 2
	d1 = Drone("human", AVAILABLE_DRONES[0], x1, y1)
	d3 = Drone("human", AVAILABLE_DRONES[2], x3, y3)  # TODO change to MACHINE
	if NUM_DRONES == 4:
		x2, y2 = 0, board.height / 2 + 1
		x4, y4 = board.width, board.height / 2 + 1
		d2 = Drone("human", AVAILABLE_DRONES[1], x2, y2) # TODO change to MACHINE
		d4 = Drone("human", AVAILABLE_DRONES[3], x4, y4) # TODO change to MACHINE
